...
export class ProjectsArea extends Area
{
    ...
    setProjectMenuEvents()
    {
        const panel = document.querySelector('.js-project-menu-panel')
        if(!panel) return

        const closeBtn = panel.querySelector('.js-project-menu-close')
        const prevBtn = panel.querySelector('.js-project-menu-prev')
        const nextBtn = panel.querySelector('.js-project-menu-next')
        ...
        if(prevBtn)
            ...
            {
                const newIndex = (this.navigation.index - 1 + projectsData.length) % projectsData.length
                this.changeProject(newIndex, ProjectsArea.DIRECTION_PREVIOUS, true, false)
            })

        if(nextBtn)
            nextBtn.addEventListener('click', () =>
            {
                const newIndex = (this.navigation.index + 1) % projectsData.length
                this.changeProject(newIndex, ProjectsArea.DIRECTION_NEXT, true, false)
            })
    }
    ...
    showProjectMenu()
    {
        const panel = document.querySelector('.js-project-menu-panel')
        if(!panel) return
        panel.classList.add('is-displayed')
    ...
    hideProjectMenu()
    {
        ...
        {
            panel.classList.remove('is-displayed')
    ...
    {
        const panel = document.querySelector('.js-project-menu-panel')
        if(!panel || !this.navigation.current) return

        const categoryEl = panel.querySelector('.js-pm-category')
        const titleEl = panel.querySelector('.js-pm-title')
        const counterEl = panel.querySelector('.js-pm-counter')
        const bodyEl = panel.querySelector('.js-pm-body')

        const project = this.navigation.current

        if(categoryEl) categoryEl.textContent = project.attributes?.at || ''
        if(titleEl) titleEl.textContent = project.displayTitle || project.title
        if(counterEl) counterEl.textContent = `${this.navigation.index + 1} / ${projectsData.length}`
        if(bodyEl) bodyEl.innerHTML = project.description || ''
    }
